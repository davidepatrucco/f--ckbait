...
describe('E01-009/010 legacy user compatibility (shim)', () => {
    ...
    it('reads a legacy flat-attribute user into the lemonsqueezer entitlement', () => {
        const legacy = { id: 'u1', email: 'a@b.c', plan: 'free', trial_remaining: 0, usage_used: 7, usage_limit: 10, usage_reset_date: future };
    ...
    it('legacy user at limit cannot summarize (no free reset from migration)', () => {
        const legacy = { id: 'u2', email: 'a@b.c', plan: 'free', trial_remaining: 0, usage_used: 10, usage_limit: 10, usage_reset_date: future };
        const user = formatUserFromDynamoDB(legacy);
        assert.equal(canUserSummarize(user, 'lemonsqueezer').canSummarize, false);
    });

    it('legacy premium user stays premium', () => {
        const legacy = { id: 'u3', email: 'a@b.c', plan: 'premium', trial_remaining: 0, usage_used: 999, usage_limit: 10, usage_reset_date: future };
        const user = formatUserFromDynamoDB(legacy);
